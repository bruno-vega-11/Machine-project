...
El archivo `framingham.csv` ya está incluido en esta carpeta. Para descargarlo desde la fuente,
basta con bajar el CSV de Kaggle [2] y guardarlo aquí con el mismo nombre. La redistribución de
Kaggle no especifica una licencia formal; los datos originales pertenecen al Framingham Heart Study
(NIH/NHLBI) y se usan aquí sólo con fines académicos.
...
de edad del subconjunto (32 a 70 años) es ligeramente superior al de la cohorte en su reclutamiento
(28 a 62), lo que sugiere que corresponde a un examen de seguimiento posterior de los mismos
...
| `age` | numérica discreta | años | Edad en el examen basal |
| `education` | ordinal | 1-4 | Nivel educativo (proxy socioeconómico) |
| `currentSmoker` | binaria | 1 = sí, 0 = no | Fumador activo |
| `cigsPerDay` | numérica discreta | cigarrillos/día | Consumo diario de tabaco (0 si no fuma) |
...
| `totChol` | numérica discreta | mg/dL | Colesterol total |
| `sysBP` | numérica continua | mm Hg | Presión arterial sistólica |
| `diaBP` | numérica continua | mm Hg | Presión arterial diastólica |
| `BMI` | numérica continua | kg/m² | Índice de masa corporal |
| `heartRate` | numérica discreta | latidos/min | Frecuencia cardíaca en reposo |
| `glucose` | numérica discreta | mg/dL | Glucosa en sangre |
...
Documentados en detalle en [`notebooks/01-eda.ipynb`](../notebooks/01-eda.ipynb):
